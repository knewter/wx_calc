defmodule WxCalc.Window do
  ...
  def handle_button(@one, calc),   do: CalculatorServer.number(calc, 1)
  def handle_button(@two, calc),   do: CalculatorServer.number(calc, 2)
  def handle_button(@three, calc), do: CalculatorServer.number(calc, 3)
  def handle_button(@four, calc),  do: CalculatorServer.number(calc, 4)
  def handle_button(@five, calc),  do: CalculatorServer.number(calc, 5)
  def handle_button(@six, calc),   do: CalculatorServer.number(calc, 6)
  def handle_button(@seven, calc), do: CalculatorServer.number(calc, 7)
  def handle_button(@eight, calc), do: CalculatorServer.number(calc, 8)
  def handle_button(@nine, calc),  do: CalculatorServer.number(calc, 9)
  def handle_button(@zero, calc),  do: CalculatorServer.number(calc, 0)
  def handle_button(@add, calc),   do: CalculatorServer.add(calc)
  def handle_button(@subtract, calc), do: CalculatorServer.subtract(calc)
  def handle_button(@mult, calc), do: CalculatorServer.multiply(calc)
  def handle_button(@div, calc), do: CalculatorServer.divide(calc)
  def handle_button(@equal, calc), do: CalculatorServer.equals(calc)
  ...
  def setup(panel) do
    ...
    :wxTextCtrl.setEditable(display, false)
