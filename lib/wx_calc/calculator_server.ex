defmodule WxCalc.CalculatorServer do
  ...
  definit do: initial_state(%{display: "0", numbers: [], value: 0, current_operation: nil})
  ...
  defcast add, state: state do
    updated_state = update_state(state, :addition)
    new_state(updated_state)
  end

  defcast subtract, state: state do
    updated_state = update_state(state, :subtraction)
    new_state(updated_state)
  end

  defcast divide, state: state do
    updated_state = update_state(state, :division)
    new_state(updated_state)
  end

  defcast multiply, state: state do
    updated_state = update_state(state, :multiplication)
    new_state(updated_state)
  end

  defcast equals, state: state do
    updated_state = update_state(state, nil)
    new_state(updated_state)
  end
